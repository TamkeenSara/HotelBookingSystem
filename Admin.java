...
public class Admin extends User {
    ...
    protected void delete_Hotel(int hotelId) throws SQLException {
        ...
        if(rs3.next()){
            ...
            PreparedStatement stat4 = connection.prepareStatement("SET FOREIGN_KEY_CHECKS = 0; \n" +
                    "delete from hotel_owner where Owner_Id = (Select Owner_Id from hotel where Hotel_Id = ?) ; \n" +
                    "delete from location where Location_Id = (Select Location_Id from hotel where Hotel_Id = ?) ; \n" +
                    "delete from room where Room_Id in (select Room_Id from hotel_rooms where Hotel_Id = ?) ; \n" +
                    "delete from hotel_rooms where Hotel_Id = ? ;  \n" +
                    "delete from pricing where Hotel_Id = ? ;  \n" +
                    "delete from hotel_cancellation_policy where Hotel_Id = ? ; \n" +
                    "delete from booking_day where Booking_Id in (select Booking_Id from booking where Hotel_Id = ? ) ; \n" +
                    "delete from booking_details where Booking_Id in (select Booking_Id from booking where Hotel_Id = ? ) ; \n" +
